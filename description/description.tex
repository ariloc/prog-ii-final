...
\noindent donde \mintinline{c}{char *textsPath} es la ruta a la carpeta de la persona en \texttt{Textos}, y \mintinline{c}{char *filesListPath} es la ruta a \texttt{archivos.txt}, donde se escribirán la lista de archivos en \texttt{textsPath} (si los comandos se ejecutan correctamente).

Notar que la parte concreta que agregué en los comandos de \texttt{cd} y \texttt{ls} es \texttt{2\char62{}/dev/null}, que le indica al procesador de comandos que la salida estándar de errores, \texttt{stderr}, identificada un \textit{descriptor de archivo} número 2, sea redirigida a \texttt{/dev/null}. Así, los mensajes de error no se mostrarán por la salida del programa en la terminal, sino serán redirigidos a \texttt{/dev/null}, donde serán descartados. 
...
Aclarar que si bien en el programa uso \texttt{fprintf()} tomando \mintinline{c}{stderr} como argumento para imprimir los mensajes de error en la salida de errores estándar, la redirección antes utilizada en los comandos ejecutados con \texttt{system()} sólo afectan a \texttt{cd} y \texttt{ls}. Es por esto que los mensajes que yo imprima a ésta salida se mostrarán sin inconvenientes. \\
